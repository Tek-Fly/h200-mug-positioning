...
class RunPodDeployer:
    ...
    def __init__(
        self, client: RunPodClient, volume_manager: Optional[VolumeManager] = None
    ):
        """Initialize deployer"""
        self.client = client
    ...
    async def deploy(self, config: DeploymentConfig) -> Dict[str, Any]:
        ...
        if not validation_result["valid"]:
            raise ValueError(
                f"Invalid deployment configuration: {validation_result['errors']}"
            )
    ...
    async def deploy_serverless(self, config: DeploymentConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            existing = next((ep for ep in endpoints if ep["name"] == config.name), None)
            ...
            ready = await self.client.wait_for_endpoint_ready(
                deployment_id, timeout=300, callback=self._deployment_progress_callback
            )
            ...
            raise RuntimeError(f"Serverless deployment timed out after 300 seconds")
        except Exception as e:
            logger.error(f"Unexpected error deploying serverless endpoint: {e}")
            raise RuntimeError(f"Failed to deploy serverless endpoint: {e}")
    ...
    async def deploy_timed(self, config: DeploymentConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            existing = next((pod for pod in pods if pod["name"] == config.name), None)
            ...
            logger.error(f"Deployment timeout: {e}")
            raise RuntimeError(f"Timed deployment timed out after 300 seconds")
        except Exception as e:
            logger.error(f"Unexpected error deploying timed pod: {e}")
            raise RuntimeError(f"Failed to deploy timed pod: {e}")
    ...
    async def deploy_both(self, config: DeploymentConfig) -> Dict[str, Any]:
        ...
        except (RunPodAPIError, RuntimeError, asyncio.TimeoutError) as e:
        ...
        return {
            ...
            "status": (
                ...
                if not errors
                else DeploymentStatus.PARTIALLY_FAILED
            ),
            "created_at": datetime.utcnow().isoformat(),
        }

    async def update_deployment(
        self, deployment_id: str, mode: DeploymentMode, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"RunPod API error updating deployment: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error updating deployment: {e}")
            raise RuntimeError(f"Failed to update deployment: {e}")

    async def stop_deployment(
        self, deployment_id: str, mode: DeploymentMode
    ) -> Dict[str, Any]:
        """Stop deployment"""
        ...
        try:
            if mode == DeploymentMode.SERVERLESS:
                ...
                result = await self.client.update_endpoint(
                    deployment_id, {"min_workers": 0, "max_workers": 0}
                )
            else:
            ...
            logger.error(f"RunPod API error stopping deployment: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error stopping deployment: {e}")
    ...
    async def start_deployment(
        ...
        try:
            if mode == DeploymentMode.SERVERLESS:
                ...
                scaling = (
                    config.scaling if config else {"min_workers": 0, "max_workers": 3}
                )
                result = await self.client.update_endpoint(
                    deployment_id,
                    {
            ...
            return {
                ...
                "mode": mode,
                "status": (
                    DeploymentStatus.RUNNING if ready else DeploymentStatus.FAILED
                ),
                "started_at": datetime.utcnow().isoformat(),
            }

        except RunPodAPIError as e:
            logger.error(f"RunPod API error starting deployment: {e}")
            raise
        except asyncio.TimeoutError:
            logger.error(f"Deployment start timed out")
            raise RuntimeError(f"Failed to start deployment within timeout")
        except Exception as e:
            logger.error(f"Unexpected error starting deployment: {e}")
            raise RuntimeError(f"Failed to start deployment: {e}")
    ...
    async def terminate_deployment(
        ...
        try:
            ...
            logger.error(f"RunPod API error terminating deployment: {e}")
            raise
        except Exception as e:
            logger.error(f"Unexpected error terminating deployment: {e}")
            raise RuntimeError(f"Failed to terminate deployment: {e}")
    ...
    async def get_deployment_status(
        ...
        try:
            ...
            logger.error(f"RunPod API error getting deployment status: {e}")
            return {
                "id": deployment_id,
                "mode": mode,
            ...
            logger.error(f"Unexpected error getting deployment status: {e}")
            return {
                "id": deployment_id,
                "mode": mode,
                "status": DeploymentStatus.FAILED,
    ...
    async def get_deployment_logs(
        self, deployment_id: str, mode: DeploymentMode, lines: int = 100
    ) -> str:
        """Get deployment logs"""
        try:
            ...
            logger.error(f"RunPod API error getting logs: {e}")
            return f"API Error fetching logs: {e}"
        except Exception as e:
            logger.error(f"Unexpected error getting deployment logs: {e}")
            return f"Internal error fetching logs: {e}"
