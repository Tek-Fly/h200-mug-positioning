...
async def basic_deployment_example():
    ...
    try:
        ...
        config = DeploymentConfig(
            ...
            docker_image=os.getenv("DOCKER_USERNAME", "myuser") + "/h200-mug-positioning",
            docker_tag="latest",
        ...
        # Scale deployment
        print("\nScaling deployment...")
        await manager.scale_deployment(state.deployment_id, min_workers=1, max_workers=3)
...
async def orchestration_example():
    ...
    try:
        ...
        config.docker_image = os.getenv("DOCKER_USERNAME", "myuser") + "/h200-mug-positioning"
        config.env_vars = {
            "MONGODB_ATLAS_URI": get_secret("MONGODB_ATLAS_URI"),
            "REDIS_HOST": get_secret("REDIS_HOST"),
            "REDIS_PASSWORD": get_secret("REDIS_PASSWORD"),
