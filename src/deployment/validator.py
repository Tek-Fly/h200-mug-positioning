...
class DeploymentValidator:
    ...
    def _validate_gpu_config(self, gpu_config) -> List[str]:
        ...
        if gpu_config.count > gpu_limits["max_count"]:
            errors.append(
                f"GPU count {gpu_config.count} exceeds maximum {gpu_limits['max_count']} for {gpu_config.type}"
            )
        ...
        if gpu_config.memory_gb and gpu_config.memory_gb < gpu_limits["min_memory_gb"]:
            errors.append(
                f"GPU memory requirement {gpu_config.memory_gb}GB is less than minimum {gpu_limits['min_memory_gb']}GB for {gpu_config.type}"
            )
    ...
    def _validate_resource_config(self, resource_config) -> List[str]:
        ...
        if resource_config.cpu_cores > limits["max_cpu_cores"]:
            errors.append(
                f"CPU cores {resource_config.cpu_cores} exceeds maximum {limits['max_cpu_cores']}"
            )

        if resource_config.memory_gb > limits["max_memory_gb"]:
            errors.append(
                f"Memory {resource_config.memory_gb}GB exceeds maximum {limits['max_memory_gb']}GB"
            )

        if resource_config.container_disk_gb > limits["max_container_disk_gb"]:
            errors.append(
                f"Container disk {resource_config.container_disk_gb}GB exceeds maximum {limits['max_container_disk_gb']}GB"
            )

        if resource_config.volume_size_gb > limits["max_volume_size_gb"]:
            errors.append(
                f"Volume size {resource_config.volume_size_gb}GB exceeds maximum {limits['max_volume_size_gb']}GB"
            )

        if (
            resource_config.network_volume_size_gb
            and resource_config.network_volume_size_gb
            > limits["max_network_volume_size_gb"]
        ):
            errors.append(
                f"Network volume size {resource_config.network_volume_size_gb}GB exceeds maximum {limits['max_network_volume_size_gb']}GB"
            )
    ...
    def _validate_scaling_config(self, scaling_config) -> List[str]:
        ...
        if scaling_config.max_workers > limits["max_workers"]:
            errors.append(
                f"Max workers {scaling_config.max_workers} exceeds maximum {limits['max_workers']}"
            )
        ...
        if (
            scaling_config.target_requests_per_second
            > limits["max_requests_per_second"]
        ):
            errors.append(
                f"Target RPS {scaling_config.target_requests_per_second} exceeds maximum {limits['max_requests_per_second']}"
            )

        if scaling_config.max_requests_per_worker > limits["max_requests_per_worker"]:
            errors.append(
                f"Max requests per worker {scaling_config.max_requests_per_worker} exceeds maximum {limits['max_requests_per_worker']}"
            )
        ...
        for key in env_vars:
            ...
            if any(pattern in key_lower for pattern in sensitive_patterns):
                if len(env_vars[key]) < 8:
                    errors.append(
                        f"Sensitive environment variable {key} appears too short"
                    )
    ...
    def _validate_network_config(self, network_config) -> List[str]:
        ...
        for port_config in network_config.ports:
            ...
            if port in reserved_ports:
                warnings.append(
                    f"Port {port} is commonly reserved, consider using a different port"
                )

            if port < 1024:
                warnings.append(
                    f"Port {port} is in privileged range, may require special permissions"
                )
    ...
    def _validate_security_config(self, security_config) -> List[str]:
        ...
        if (
            not security_config.enable_jwt_auth
            and not security_config.enable_api_key_auth
        ):
            warnings.append(
                "No authentication method enabled, API will be publicly accessible"
            )

        if not security_config.enable_rate_limiting:
            warnings.append("Rate limiting is disabled, API may be vulnerable to abuse")

        if security_config.enable_tls and not (
            security_config.tls_cert_path and security_config.tls_key_path
        ):
            warnings.append("TLS is enabled but certificate paths are not provided")

        if security_config.max_requests_per_minute > 1000:
            warnings.append(
                f"Rate limit of {security_config.max_requests_per_minute} requests/minute is very high"
            )
    ...
    async def validate_deployment_health(
            ...
            try:
                if mode == DeploymentMode.SERVERLESS:
                    ...
                    results["checks"]["status"] = {
                        ...
                        "details": {"status": pod.get("status")},
                    }
            except Exception as e:
                results["checks"]["status"] = {"passed": False, "error": str(e)}
        ...
        if "connectivity" in checks and mode == DeploymentMode.TIMED:
            try:
                ...
                results["checks"]["connectivity"] = {
                    "passed": has_ip,
                    "details": {"ip": pod.get("ip", "none")},
                }
            except Exception as e:
                results["checks"]["connectivity"] = {"passed": False, "error": str(e)}
                results["healthy"] = False
        ...
        if "resources" in checks and mode == DeploymentMode.TIMED:
            try:
                ...
                resources_healthy = (
                    cpu_usage < 90 and memory_usage < 90 and gpu_usage < 95
                )
                ...
                results["checks"]["resources"] = {"passed": False, "error": str(e)}
        ...
        if "performance" in checks and mode == DeploymentMode.SERVERLESS:
            try:
                ...
                performance_healthy = (
                    avg_latency < 1000  # Less than 1 second
                    and error_rate < 0.05  # Less than 5% errors
                )
                ...
                results["checks"]["performance"] = {"passed": False, "error": str(e)}
    ...
    async def validate_rollback_safety(
        self, current_config: DeploymentConfig, target_config: DeploymentConfig
    ) -> Dict[str, Any]:
        ...
        if current_config.gpu.type != target_config.gpu.type:
            warnings.append(
                f"GPU type will change from {current_config.gpu.type} to {target_config.gpu.type}"
            )

        if current_config.gpu.count != target_config.gpu.count:
            warnings.append(
                f"GPU count will change from {current_config.gpu.count} to {target_config.gpu.count}"
            )

        # Check resource changes
        if target_config.resources.memory_gb < current_config.resources.memory_gb:
            warnings.append(
                f"Memory will decrease from {current_config.resources.memory_gb}GB to {target_config.resources.memory_gb}GB"
            )

        if target_config.resources.cpu_cores < current_config.resources.cpu_cores:
            warnings.append(
                f"CPU cores will decrease from {current_config.resources.cpu_cores} to {target_config.resources.cpu_cores}"
            )
        ...
        if missing_vars:
            errors.append(
                f"Target configuration missing environment variables: {missing_vars}"
            )

        # Check scaling changes (for serverless)
        if (
            current_config.mode == DeploymentMode.SERVERLESS
            and target_config.mode == DeploymentMode.SERVERLESS
        ):
