...
class RunPodClient:
    ...
    async def update_endpoint(
        self, endpoint_id: str, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Update serverless endpoint"""
        return await self._request(
            "PATCH", f"/serverless/endpoints/{endpoint_id}", json=config
        )
    ...
    async def get_pod_logs(self, pod_id: str, lines: int = 100) -> str:
        ...
        response = await self._request(
            "GET", f"/pods/{pod_id}/logs", params={"lines": lines}
        )
    ...
    async def create_volume(
        self, name: str, size_gb: int, region: str = "us-east-1"
    ) -> Dict[str, Any]:
        """Create network volume"""
        config = {"name": name, "size": size_gb, "region": region}
        return await self._request("POST", "/volumes", json=config)

    async def update_volume(self, volume_id: str, size_gb: int) -> Dict[str, Any]:
        """Update volume size"""
        return await self._request(
            "PATCH", f"/volumes/{volume_id}", json={"size": size_gb}
        )
    ...
    async def attach_volume(
        self, pod_id: str, volume_id: str, mount_path: str = "/workspace"
    ) -> Dict[str, Any]:
        """Attach volume to pod"""
        config = {"volume_id": volume_id, "mount_path": mount_path}
    ...
    async def get_endpoint_metrics(
        ...
        return await self._request(
            "GET", f"/serverless/endpoints/{endpoint_id}/metrics", params=params
        )
    ...
    async def register_webhook(
        self, endpoint_id: str, webhook_url: str, events: List[str] = None
    ) -> Dict[str, Any]:
        """Register webhook for endpoint events"""
        if events is None:
            events = ["job.completed", "job.failed", "worker.started", "worker.stopped"]

        config = {"url": webhook_url, "events": events}
        return await self._request(
            "POST", f"/serverless/endpoints/{endpoint_id}/webhooks", json=config
        )

    async def list_webhooks(self, endpoint_id: str) -> List[Dict[str, Any]]:
        """List webhooks for endpoint"""
        response = await self._request(
            "GET", f"/serverless/endpoints/{endpoint_id}/webhooks"
        )
    ...
    async def delete_webhook(self, endpoint_id: str, webhook_id: str) -> Dict[str, Any]:
        """Delete webhook"""
        return await self._request(
            "DELETE", f"/serverless/endpoints/{endpoint_id}/webhooks/{webhook_id}"
        )

    # Helper methods

    async def wait_for_pod_ready(
        self, pod_id: str, timeout: int = 300, callback: Optional[Callable] = None
    ) -> bool:
    ...
    async def wait_for_endpoint_ready(
        self, endpoint_id: str, timeout: int = 300, callback: Optional[Callable] = None
    ) -> bool:
        ...
        while (datetime.utcnow() - start_time).total_seconds() < timeout:
            try:
                ...
                if status == "running" and workers.get("running", 0) > 0:
                    logger.info(
                        f"Endpoint {endpoint_id} is ready with {workers['running']} workers"
                    )
                    return True
                elif status == "failed":
