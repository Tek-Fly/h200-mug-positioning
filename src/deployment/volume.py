...
class VolumeManager:
    ...
    async def ensure_volume(
        ...
        try:
            ...
            existing = next((vol for vol in volumes if vol["name"] == name), None)
            ...
            if existing:
                ...
                if current_size < size_gb:
                    logger.info(
                        f"Resizing volume {volume_id} from {current_size}GB to {size_gb}GB"
                    )
        ...
        for volume in volumes:
            ...
            try:
                ...
                if age_days > days_unused:
                    logger.info(
                        f"Deleting unused volume {volume['id']} (unused for {age_days} days)"
                    )
    ...
    async def attach_to_deployment(
        self, volume_id: str, deployment_id: str, mount_path: str = "/models"
    ) -> Dict[str, Any]:
        ...
        try:
            result = await self.client.attach_volume(
                deployment_id, volume_id, mount_path
            )
    ...
    async def detach_from_deployment(
        self, volume_id: str, deployment_id: str
    ) -> Dict[str, Any]:
    ...
    async def sync_models_to_volume(
        self, volume_id: str, models: List[str]
    ) -> Dict[str, Any]:
    ...
    async def restore_volume_snapshot(
        self, snapshot_id: str, volume_name: str
    ) -> Dict[str, Any]:
