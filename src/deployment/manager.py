...
class DeploymentManager:
    ...
    async def update_deployment(self, deployment_id: str, 
                               updates: Dict[str, Any]) -> DeploymentState:
    ...
    async def list_deployments(self, mode: Optional[DeploymentMode] = None,
                              status: Optional[DeploymentStatus] = None) -> List[DeploymentState]:
    ...
    async def get_deployment_metrics(self, deployment_id: str) -> Dict[str, Any]:
        ...
        async with self.client:
            # Get current status
            status = await self.deployer.get_deployment_status(deployment_id, state.mode)
    ...
    async def get_deployment_logs(self, deployment_id: str, lines: int = 100) -> str:
        ...
        async with self.client:
            return await self.deployer.get_deployment_logs(deployment_id, state.mode, lines)
    
    async def scale_deployment(self, deployment_id: str, min_workers: int, max_workers: int) -> DeploymentState:
    ...
    async def health_check(self, deployment_id: str) -> Dict[str, Any]:
        ...
        async with self.client:
            ...
            if state.mode == DeploymentMode.SERVERLESS:
                ...
                health["checks"]["error_rate"] = self._calculate_error_rate(status["metrics"])
                
            else:  # Timed
                metrics = status.get("metrics", {})
                health["checks"]["cpu_healthy"] = metrics.get("cpu_usage_percent", 0) < 90
                health["checks"]["memory_healthy"] = metrics.get("memory_usage_mb", 0) < (state.config.resources.memory_gb * 900)  # 90% threshold
                health["checks"]["gpu_healthy"] = metrics.get("gpu_usage_percent", 0) < 95
    ...
    async def _add_deployment_event(self, deployment_id: str, event_type: str, data: Dict[str, Any]):
            ...
            try:
                async with self.client:
                    ...
                    for deployment_id, state in list(self._deployments.items()):
                        if state.status not in [DeploymentStatus.RUNNING, DeploymentStatus.STARTING]:
                            continue
    ...
    async def _save_deployment_states(self):
        ...
        try:
            ...
            logger.error(f"Failed to save deployment states: {e}")
    
    async def get_deployment_costs(self, deployment_id: str, 
        ...
        # Additional costs
        storage_gb = state.config.resources.volume_size_gb + (state.config.resources.network_volume_size_gb or 0)
        storage_cost = storage_gb * 0.10 / 730  # $0.10/GB/month
        
        # Bandwidth (estimated)
        bandwidth_cost = 0.01 * runtime_hours  # Simplified estimate
        
        total_cost = (hourly_rate * runtime_hours) + (storage_cost * runtime_hours) + bandwidth_cost
