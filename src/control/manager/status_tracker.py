...
class StatusTracker:
    ...
    def clear_alerts(self, server_id: str, alert_type: Optional[str] = None):
        ...
        if alert_type:
            ...
            deployment.active_alerts = [
                a for a in deployment.active_alerts if a.get("type") != alert_type
            ]
        else:
            # Clear all
        ...
        for check_name, check_func in checks:
            try:
                ...
                if health_check.status == HealthStatus.UNHEALTHY:
                    ...
                    and overall_health == HealthStatus.HEALTHY
                ):
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        for state in ServerState:
            state_counts[state.value] = sum(1 for d in deployments if d.state == state)
