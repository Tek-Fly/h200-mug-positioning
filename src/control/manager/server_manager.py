...
class ServerManager:
    ...
    async def _deployment_state_to_server_info(self, state) -> ServerInfo:
        ...
        server_config = ServerConfig(
            ...
            min_instances=state.config.scaling.min_workers if state.config.scaling else 0,
            max_instances=state.config.scaling.max_workers if state.config.scaling else 1,
        ...
        return ServerInfo(
            ...
            started_at=state.created_at if state.status == DeploymentStatus.RUNNING else None,
            endpoint_url=metrics.get("endpoint_url") if metrics else None,
            metrics_url=metrics.get("metrics_url") if metrics else None,
            current_instances=metrics.get("workers", {}).get("running", 0) if metrics else 0,
    ...
    def _server_config_to_deployment_config(self, 
                                          server_type: ServerType,
                                          config: ServerConfig,
                                          tags: Optional[Dict[str, str]] = None) -> DeploymentConfig:
    ...
    async def list_servers(self, server_type: Optional[ServerType] = None) -> List[ServerInfo]:
