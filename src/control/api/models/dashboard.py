...
class CostMetrics(BaseModel):
    ...
    breakdown: Dict[str, float] = Field(
        default_factory=dict, description="Detailed cost breakdown"
    )
...
class DashboardRequest(BaseModel):
    ...
    time_range: TimeRange = Field(
        default=TimeRange.LAST_HOUR, description="Time range for metrics"
    )
    activity_limit: int = Field(
        default=50, ge=1, le=1000, description="Number of activity entries"
    )
...
class DashboardResponse(BaseModel):
    ...
    # Service health
    services: Optional[List[ServiceHealth]] = Field(
        None, description="Service health status"
    )
    overall_health: Optional[ServiceStatus] = Field(
        None, description="Overall system health"
    )
    ...
    metrics: Optional[List[SystemMetric]] = Field(
        None, description="Additional system metrics"
    )
    ...
    # Recent activity
    recent_activity: Optional[List[ActivityLog]] = Field(
        None, description="Recent activity log"
    )
