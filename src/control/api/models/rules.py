...
class RuleAction(BaseModel):
    ...
    type: str = Field(
        ..., description="Action type (e.g., 'adjust', 'alert', 'reject')"
    )
...
class NaturalLanguageRuleRequest(BaseModel):
    ...
    auto_enable: bool = Field(
        default=False, description="Automatically enable the rule"
    )
...
class NaturalLanguageRuleResponse(BaseModel):
    ...
    interpretation: str = Field(
        ..., description="How the system interpreted the request"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Interpretation confidence"
    )
    warnings: List[str] = Field(
        default_factory=list, description="Any warnings or ambiguities"
    )
...
class RuleEvaluationRequest(BaseModel):
    ...
    rule_ids: Optional[List[str]] = Field(
        None, description="Specific rules to evaluate"
    )
...
class RuleEvaluationResult(BaseModel):
    ...
    actions_taken: List[RuleAction] = Field(
        ..., description="Actions that were/would be taken"
    )
...
class RuleEvaluationResponse(BaseModel):
    ...
    results: List[RuleEvaluationResult] = Field(
