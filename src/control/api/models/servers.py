...
class ServerConfig(BaseModel):
    ...
    min_instances: int = Field(
        default=0, ge=0, description="Minimum instances (serverless)"
    )
    max_instances: int = Field(
        default=10, ge=1, description="Maximum instances (serverless)"
    )
    target_utilization: float = Field(
        default=0.7, ge=0.1, le=1.0, description="Target GPU utilization"
    )

    # Timeout configuration
    idle_timeout_seconds: int = Field(
        default=600, ge=60, description="Idle timeout before shutdown"
    )
    max_runtime_seconds: Optional[int] = Field(
        None, description="Maximum runtime (timed instances)"
    )
...
class ServerControlRequest(BaseModel):
    ...
    config: Optional[ServerConfig] = Field(
        None, description="New configuration (for update/scale)"
    )
    force: bool = Field(default=False, description="Force action even if risky")
    wait_for_completion: bool = Field(
        default=True, description="Wait for action to complete"
    )
...
class DeploymentRequest(BaseModel):
    ...
    auto_start: bool = Field(
        default=True, description="Start immediately after deployment"
    )
