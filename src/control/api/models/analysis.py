...
class PositioningResult(BaseModel):
    ...
    position: str = Field(..., description="Position description (e.g., 'left edge', 'center')")
    confidence: float = Field(..., ge=0.0, le=1.0, description="Position confidence")
    offset_pixels: Point2D = Field(..., description="Offset from ideal position in pixels")
    offset_mm: Optional[Point2D] = Field(None, description="Offset in millimeters if calibrated")
    rule_violations: List[str] = Field(default_factory=list, description="Violated rules")


class AnalysisRequest(BaseModel):
    """Image analysis request."""
    include_feedback: bool = Field(default=True, description="Include positioning feedback")
    rules_context: Optional[str] = Field(None, description="Natural language rules context")
    calibration_mm_per_pixel: Optional[float] = Field(None, description="Calibration factor")
...
class AnalysisResponse(BaseModel):
    ...
    suggestions: List[str] = Field(default_factory=list, description="Positioning suggestions")
...
class AnalysisFeedback(BaseModel):
    ...
    correct_position: Optional[str] = Field(None, description="Correct position if different")
...
class BatchAnalysisRequest(BaseModel):
    ...
    settings: AnalysisRequest = Field(default_factory=AnalysisRequest, description="Analysis settings")
...
class BatchAnalysisResponse(BaseModel):
    ...
    errors: List[Dict[str, str]] = Field(default_factory=list, description="Error details for failed images")