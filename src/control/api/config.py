...
class Settings(BaseSettings):
    ...
    allowed_hosts: Optional[List[str]] = Field(
        default=None, description="Allowed hosts for TrustedHostMiddleware"
    )
    ...
    mongodb_database: str = Field(
        default="h200_positioning", description="MongoDB database name"
    )
    ...
    # RunPod settings
    runpod_api_key: Optional[SecretStr] = Field(
        default=None, description="RunPod API key"
    )
    runpod_serverless_id: Optional[str] = Field(
        default=None, description="RunPod serverless endpoint ID"
    )
    runpod_timed_id: Optional[str] = Field(
        default=None, description="RunPod timed endpoint ID"
    )

    # Control plane settings
    idle_timeout_seconds: int = Field(
        default=600, description="Idle timeout before auto-shutdown (seconds)"
    )
    enable_auto_shutdown: bool = Field(
        default=True, description="Enable automatic shutdown of idle servers"
    )
    health_check_interval: int = Field(
        default=30, description="Health check interval (seconds)"
    )
    resource_monitor_interval: int = Field(
        default=5, description="Resource monitoring interval (seconds)"
    )
    ...
    google_secret_manager_enabled: bool = Field(
        default=False, description="Enable Google Secret Manager"
    )

    # Cloudflare R2 settings
    r2_access_key_id: Optional[SecretStr] = Field(
        default=None, description="R2 access key ID"
    )
    r2_secret_access_key: Optional[SecretStr] = Field(
        default=None, description="R2 secret access key"
    )
