...
class ModelConfig:
    ...
    def __post_init__(self):
        ...
        if self.device.startswith("cuda") and not torch.cuda.is_available():
            logger.warning(
                "CUDA not available, falling back to CPU", requested_device=self.device
            )
            self.device = "cpu"
...
class BaseModel(ABC):
    ...
    def __init__(self, config: ModelConfig, cache: Optional[DualLayerCache] = None):
        """
        ...
        self._state_file = Path(
            f"/tmp/h200_model_state_{config.name}_{config.version}.json"
        )
    ...
    async def load(self) -> None:
        ...
        try:
            ...
            logger.error("Model loading failed", name=self.config.name, error=str(e))
            raise
    ...
    async def _warmup(self) -> None:
        ...
        try:
            ...
            self.state = ModelState.ERROR
            logger.error("Model warmup failed", name=self.config.name, error=str(e))
            raise
    ...
    async def _create_dummy_input(self) -> Any:
        ...
        return torch.randn(
            1, 3, 224, 224, device=self.config.device, dtype=self.config.dtype
        )

    async def predict(self, inputs: Any, batch_size: Optional[int] = None) -> Any:
        """
    ...
    async def download_from_r2(self, local_path: Optional[str] = None) -> str:
        ...
        # Download model
        await r2_client.download_file(self.config.r2_key, local_path)
    ...
    async def upload_to_r2(self, local_path: str) -> str:
        ...
        logger.info("Model uploaded to R2", name=self.config.name, r2_key=r2_key)
    ...
    def get_gpu_allocation(self) -> Dict[str, Any]:
        ...
        if not torch.cuda.is_available():
            return {"device": "cpu", "allocated_mb": 0, "reserved_mb": 0, "free_mb": 0}
            ...
            stats.update(
                {
                    ...
                    "avg_batch_size": (
                        np.mean(self._batch_sizes) if self._batch_sizes else 0
                    ),
                    "avg_gpu_memory_mb": (
                        np.mean(self._gpu_memory_usage) if self._gpu_memory_usage else 0
                    ),
                }
            )
    ...
    async def _load_state(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning(
                "Failed to load model state", name=self.config.name, error=str(e)
            )
            return None
