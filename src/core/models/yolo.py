...
class YOLOv8Model(BaseModel):
    ...
    async def _load_model_impl(self) -> torch.nn.Module:
        ...
        if not model_path.exists():
            ...
            try:
                ...
                logger.warning(f"R2 download failed: {e}. Downloading from ultralytics...")
    ...
    async def preprocess(self, inputs: Union[Image.Image, List[Image.Image], np.ndarray]) -> torch.Tensor:
        """
        ...
        for result in outputs:
            ...
            if result.boxes is not None:
                ...
                for i in range(len(boxes)):
                    ...
                    # Get class name
                    class_name = self._class_names[cls_idx] if cls_idx < len(self._class_names) else f"class_{cls_idx}"
                    
                    # Check if it's a mug-related class
                    is_mug_related = class_name.lower() in [c.lower() for c in self.MUG_CLASSES]
    ...
    def analyze_detections(self, detections: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if mug_detections:
            ...
            for det in mug_detections:
                mug_type = det['class']
                analysis['mug_types'][mug_type] = analysis['mug_types'].get(mug_type, 0) + 1
            
            # Calculate quality metrics
            confidences = [d['confidence'] for d in mug_detections]
            analysis['quality_metrics']['average_confidence'] = float(np.mean(confidences))
            ...
            if len(mug_detections) > 1:
                ...
                analysis['spatial_distribution']['average_distance'] = float(np.mean(distances))
                
                # Simple clustering score (inverse of average distance normalized)
                max_distance = np.sqrt(640**2 + 640**2)  # Diagonal of typical image
                analysis['spatial_distribution']['clustering_score'] = 1.0 - (analysis['spatial_distribution']['average_distance'] / max_distance)
            ...
            analysis['spatial_distribution']['coverage_ratio'] = min(total_area / image_area, 1.0)
