...
class ModelManager:
    ...
    async def preload_models(self, model_names: List[str]) -> None:
        ...
        logger.info(
            ...
            failed=[name for name, r in zip(model_names, results) if isinstance(r, Exception)]
        )
    ...
    async def get_optimal_model(
        ...
        candidates.sort(
            key=lambda x: sum(x[1].performance_profile.values()),
            reverse=True
        )
        ...
        for source_name, download_func in self.download_sources.items():
            try:
                ...
                if model_path.exists():
                    logger.info(f"Successfully downloaded {model_name} from {source_name}")
    ...
    async def _download_from_huggingface(self, model_name: str, target_path: Path) -> None:
    ...
    async def _download_from_github_lfs(self, model_name: str, target_path: Path) -> None:
    ...
    async def _download_from_http(self, model_name: str, target_path: Path, url: Optional[str] = None) -> None:
    ...
    async def _save_benchmarks(self) -> None:
        ...
        for b in self._benchmarks:
            data = {
                ...
                'avg_inference_ms': np.mean(b.inference_times_ms) if b.inference_times_ms else 0,
    ...
    def get_registry_info(self) -> Dict[str, Any]:
        ...
        for name, metadata in self.REGISTRY.items():
            info[name] = {
                ...
                'loaded': name in self._models and self._models[name].state == ModelState.READY
            }
