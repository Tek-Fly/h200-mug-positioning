...
class CLIPVisionModel(BaseModel):
    ...
    def __init__(
        ...
        if model_name not in self.AVAILABLE_MODELS:
            raise ValueError(f"Invalid model: {model_name}. Must be one of {list(self.AVAILABLE_MODELS.keys())}")
    ...
    async def preprocess(self, inputs: Union[Image.Image, List[Image.Image], str, List[str]]) -> Any:
        """
    ...
    async def generate_positioning_embedding(
        ...
        if context:
            ...
            if context_prompts:
                context_embeddings = await self.encode_text(context_prompts, return_tensor=True)
                ...
                combined_embedding = alpha * base_embedding + (1 - alpha) * context_embedding
