...
class RuleStorage:
    ...
    async def _create_indexes(self):
        ...
        await self.rules_collection.create_index([("name", "text"), ("description", "text")])
    ...
    async def get_rule(self, rule_id: str) -> Optional[Rule]:
        ...
        try:
            rule_dict = await self.rules_collection.find_one(
    ...
    async def batch_create_rules(
        ...
        for rule in rules:
            try:
                ...
                logger.error(
                    "Failed to create rule in batch",
                    rule_name=rule.name,
                    error=str(e)
                )
