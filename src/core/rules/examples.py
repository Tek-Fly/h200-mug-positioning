...
async def example_rule_application():
    ...
    await rule_engine.add_rule("Group all drinks together on the right side", priority=5)
...
async def example_rule_management():
    ...
    result = await engine.add_rule(
        "Keep beverages at least 10 inches from electronic devices",
        priority=8
    )
    ...
    if result.valid:
        ...
        for entry in history[:3]:
            print(f"- Version {entry['version']}: {entry['action']} by {entry['user_id']}")
