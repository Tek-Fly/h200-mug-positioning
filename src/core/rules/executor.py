...
from .models import (
    Rule, RuleExecutionResult, RuleAction,
...
class RuleExecutor:
        ...
        for pos in positions:
            for avoid in avoid_objects:
                ...
                if distance < min_distance:
                    ...
                    pos.x = new_x
                    pos.y = new_y
                    pos.reasoning += f" | Moved {move_distance:.0f}px from {from_object}"
                    modified = True
    ...
    def _execute_place_on(
        ...
        surfaces = [
            s for s in context['surfaces']
            if s['type'].lower() == surface_type.lower()
        ]
        ...
        for i, pos in enumerate(positions):
            ...
            best_surface = min(
                surfaces,
                key=lambda s: self._distance_to_surface(pos, s)
            )
    ...
    def _is_on_surface(
        ...
        bbox = surface['bbox']
        return (
            bbox['x1'] <= position.x <= bbox['x2'] and
            bbox['y1'] <= position.y <= bbox['y2']
        )
        ...
        for i, pos in enumerate(positions):
            if i < len(positions) - 1:
                nearest_idx = min(
                    range(len(positions)),
                    key=lambda j: (
                        relationships['distances'].get((min(i,j), max(i,j)), float('inf'))
    ...
    def _generate_recommendations(
        ...
        if total_conflicts > 0:
            recommendations.append(
                f"Resolved {total_conflicts} positioning conflicts."
            )
