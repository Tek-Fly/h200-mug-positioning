...
# First-party imports
from src.core.positioning import MugPositioningEngine, PositioningResult

# Local imports
from .executor import RuleExecutor
from .models import Rule, RuleExecutionResult, RuleType, RuleValidationResult
...
class RuleEngine:
    ...
    def __init__(
        ...
        logger.info("RuleEngine initialized", conflict_resolution=conflict_resolution)
    ...
    async def initialize(self):
        ...
        if not self.initialized:
            ...
            logger.info(
                "RuleEngine fully initialized", active_rules=len(self.rule_cache)
            )
    ...
    async def add_rule(
        ...
        try:
            ...
            return RuleValidationResult(
                valid=False, errors=[f"Failed to add rule: {str(e)}"]
            )

    async def update_rule(
        self, rule_id: str, natural_language: str, user_id: str = "system"
    ) -> RuleValidationResult:
        """
        ...
        if not existing_rule:
            return RuleValidationResult(valid=False, errors=["Rule not found"])
        ...
        if success:
            ...
            logger.info(
                "Rule updated successfully", rule_id=rule_id, version=new_rule.version
            )

        return validation_result

    async def delete_rule(
        self, rule_id: str, user_id: str = "system", permanent: bool = False
    ) -> bool:
        """
        Delete a rule.
        ...
        success = await self.storage.delete_rule(
            rule_id, user_id, soft_delete=not permanent
        )
        ...
        if success:
            ...
            logger.info("Rule deleted", rule_id=rule_id, permanent=permanent)
    ...
    async def list_rules(
        ...
        return await self.storage.list_rules(filter_dict=filter_dict, limit=limit)
    ...
    async def apply_rules(
        ...
        # Get rules to apply
        if rule_ids:
            rules = [await self.get_rule(rid) for rid in rule_ids]
            rules = [r for r in rules if r]  # Filter None values
        else:
            # Use all active rules from cache
            rules = [rule for rule in self.rule_cache.values() if rule.enabled]
        ...
        updated_result, execution_results = await self.executor.execute_rules(
            rules, positioning_result, scene_context, image.size
        )
    ...
    async def validate_rule(self, natural_language: str) -> RuleValidationResult:
        """
    ...
    async def batch_add_rules(
        self, rule_texts: List[str], user_id: str = "system"
    ) -> Dict[str, Any]:
        """
        ...
        # Process results
        for i, (rule_text, validation) in enumerate(
            zip(rule_texts, validation_results)
        ):
            if validation.valid:
                try:
                    rule_id = await self.storage.create_rule(
                        validation.parsed_rule, user_id
                    )
                    ...
                    results["errors"].append(
                        {"index": i, "text": rule_text[:100], "error": str(e)}
                    )
            else:
                results["failed"] += 1
                results["errors"].append(
    ...
    async def get_rule_suggestions(self, scene_context: Dict[str, Any]) -> List[str]:
        """
        ...
        # Edge detection
        if any(
            "edge" in zone.get("type", "")
            for zone in scene_context.get("danger_zones", [])
        ):
    ...
    async def export_rules(
        self, format: str = "json", include_disabled: bool = False
    ) -> Any:
        """
    ...
    async def import_rules(
        ...
        result = await self.storage.import_rules(rules_data, user_id, overwrite)
    ...
    async def _load_active_rules(self):
        ...
        self.rule_cache = {rule.id: rule for rule in active_rules if rule.id}

        logger.info("Active rules loaded", count=len(self.rule_cache))
