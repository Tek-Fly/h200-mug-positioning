...
class MCPClient:
    ...
    def _get_auth_data(self) -> Dict[str, Any]:
        ...
        if self.auth_type == MCPAuthType.JWT:
            ...
            return {
                "type": MCPAuthType.API_KEY.value,
                "key": self.auth_token
            }
    ...
    async def get_capabilities(self) -> MCPCapabilities:
        ...
        async with self._session.get(
            f"{self.base_url}/mcp/v1/capabilities",
    ...
    async def get_suggestions(
        ...
        if not response.result.success:
            raise Exception(f"Suggestion generation failed: {response.result.error.message}")
    ...
    async def update_positioning(
        ...
        if not response.result.success:
            raise Exception(f"Positioning update failed: {response.result.error.message}")
        ...
        if suggestions['suggestions']:
            feedback_result = await client.update_positioning(
                ...
                feedback={
                    ...
                    "suggestions_accepted": [s['mug_id'] for s in suggestions['suggestions'][:2]],
