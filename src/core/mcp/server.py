...
class MCPServer:
    ...
    async def _register_tools(self):
        ...
        tool_instances = {
            "analyze_image": AnalyzeImageTool(
                analyzer=self.analyzer, authenticator=self.authenticator
            ),
            "apply_rules": ApplyRulesTool(
                rule_engine=self.rule_engine, authenticator=self.authenticator
            ),
    ...
    async def _start_websocket_server(self):
        ...
        self._websocket_server = await websockets.serve(
            handler, self.host, self.port, ping_interval=30, ping_timeout=10
        )
                ...
                try:
                    ...
                    if data.get("type") == "capabilities":
                        ...
                        await websocket.send(
                            json.dumps(
                                {"type": "capabilities", "data": capabilities.dict()}
                            )
                        )
                        ...
                        await websocket.send(
                            json.dumps({"type": "response", "data": response.dict()})
                        )
    ...
    async def _start_http_server(self):
        ...
        logger.info("HTTP server started", host=self.host, port=self.port + 1)
    ...
    async def _handle_execute(self, request):
        ...
        try:
            ...
            return web.json_response(
                {"error": {"code": "REQUEST_ERROR", "message": str(e)}}, status=400
            )
    ...
    async def _handle_batch(self, request):
        ...
        try:
            ...
            return web.json_response(
                {"error": {"code": "BATCH_ERROR", "message": str(e)}}, status=400
            )
    ...
    async def _handle_batch_request(
        self, batch_request: MCPBatchRequest
    ) -> MCPBatchResponse:
        """Handle batch request."""
        responses = []
        ...
        if batch_request.parallel:
            ...
            tasks = [self._handle_request(req) for req in batch_request.requests]
                ...
                if isinstance(response, Exception):
                    responses[i] = MCPResponse(
                        ...
                        result=MCPToolResult(
                            success=False,
                            error=MCPError(
                                code="BATCH_ITEM_ERROR", message=str(response)
                            ),
                            execution_time_ms=0,
                        ),
                    )
    ...
    def get_capabilities(self) -> MCPCapabilities:
        ...
        tool_definitions = [tool.get_definition() for tool in self.tools.values()]

        return MCPCapabilities(
            version=MCPVersion.V1_0,
            tools=tool_definitions,
            auth_types=(
                [MCPAuthType.JWT, MCPAuthType.API_KEY] if self.enable_auth else []
            ),
            features=[
                "async",
                "batch",
    ...
    async def broadcast_to_clients(self, message: Dict[str, Any]):
        ...
        for client in self._connected_clients:
            try:
                ...
                logger.error(
                    "Broadcast error", error=str(e), client=client.remote_address
                )
...
async def run_mcp_server(host: str = "0.0.0.0", port: int = 8765, **kwargs):
    """Run the MCP server."""
