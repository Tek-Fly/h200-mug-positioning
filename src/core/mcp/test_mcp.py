...
async def mcp_client(mcp_server):
    ...
    token = authenticator.generate_jwt_token(
        client_id="test-client", scopes=["analyze", "suggest", "update"]
    )
...
class TestMCPAuth:
    ...
    def test_jwt_token_validation(self):
        ...
        expired_token = jwt.encode(
            {"client_id": "test-client", "exp": datetime.utcnow() - timedelta(hours=1)},
            "test-secret",
            algorithm="HS256",
        )
    ...
    async def test_auth_decorator(self):
        ...
        request = MCPRequest(tool="test", parameters={})
        ...
        request.auth = {"type": MCPAuthType.JWT.value, "token": token}
        result = await tool.protected_method(request)
...
class TestMCPServer:
    ...
    async def test_batch_request_handling(self, mcp_server):
        ...
        auth_data = {"type": MCPAuthType.JWT.value, "token": token}
        ...
        batch = MCPBatchRequest(
            requests=[
                ...
                MCPRequest(
                    tool="apply_rules",
                    parameters={"rule_text": "Test rule", "validate_only": True},
                    auth=auth_data,
                ),
            ],
            parallel=True,
    ...
    async def test_statistics(self, mcp_server):
        ...
        assert (
            mcp_server._stats["requests_total"] == initial_stats["requests_total"] + 1
        )
        assert (
            mcp_server._stats["requests_failed"] == initial_stats["requests_failed"] + 1
        )
...
async def test_integration_flow(mcp_server, test_image):
    ...
    try:
        ...
        auth = mcp_server.authenticator
        token = auth.generate_jwt_token(
            client_id="integration-test", scopes=["analyze", "suggest", "update"]
        )
