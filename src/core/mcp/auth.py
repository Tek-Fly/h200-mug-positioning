...
class MCPAuthenticator:
        ...
        while True:
            try:
                ...
                for client_id in list(self._rate_limits.keys()):
                    for window, timestamps in list(
                        self._rate_limits[client_id].items()
                    ):
    ...
    def generate_jwt_token(
        self, client_id: str, scopes: List[str] = None, metadata: Dict[str, Any] = None
    ) -> str:
    ...
    def generate_api_key(
        ...
        logger.info("Generated API key", client_id=client_id, name=name, scopes=scopes)
    ...
    async def authenticate(
        ...
        if not auth_data:
            return (
                False,
                None,
                MCPError(code="AUTH_REQUIRED", message="Authentication required"),
            )
        ...
        if auth_type == MCPAuthType.JWT.value:
            ...
            if not token:
                return (
                    False,
                    None,
                    MCPError(code="MISSING_TOKEN", message="JWT token required"),
                )
            ...
            if not api_key:
                return (
                    False,
                    None,
                    MCPError(code="MISSING_API_KEY", message="API key required"),
                )
    ...
    def check_rate_limit(
        self, client_id: str, rate_limit: Optional[Dict[str, int]] = None
        ...
        for window, limit in rate_limit.items():
            ...
            if len(valid_timestamps) >= limit:
                ...
                reset_at = datetime.fromtimestamp(
                    min(valid_timestamps) + window_seconds
                )
    ...
    def check_scope(self, required_scope: str, auth_info: Dict[str, Any]) -> bool:
    ...
    def auth_required(self, scope: Optional[str] = None, check_rate_limit: bool = True):
        """
                ...
                if not is_auth:
                    return MCPResponse(
                        ...
                        result=MCPToolResult(
                            success=False, error=error, execution_time_ms=0
                        ),
                    )
                        ...
                        return MCPResponse(
                            ...
                            result=MCPToolResult(
                                ...
                                error=MCPError(
                                    ...
                                    retry_after=int(
                                        (
                                            limit_info.reset_at - datetime.utcnow()
                                        ).total_seconds()
                                    ),
                                ),
                                execution_time_ms=0,
