...
class H200ImageAnalyzer:
    ...
    async def analyze_batch(
        ...
        for i in range(0, len(images), self.batch_size):
            ...
            chunk_metadata = metadata_list[i:i + self.batch_size] if metadata_list else [None] * len(chunk)
    ...
    async def _run_object_detection(self, image: Image.Image) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            detections_batch = await self.yolo_model.detect_mugs(
                image,
                mug_only=False  # Get all objects for scene context
            )
        ...
        if self.positioning_engine and image:
            try:
                ...
                if mug_positions and positioning_result.recommendations:
                    mug_positions[0]['recommendations'] = positioning_result.recommendations
                ...
                logger.error("Advanced positioning failed, falling back to basic", error=str(e))
    ...
    def _calculate_confidence_scores(
        self,
        detections: List[Dict[str, Any]],
        mug_positions: List[Dict[str, float]]
    ) -> Dict[str, float]:
    ...
    async def search_similar_images(
        ...
        try:
            ...
            logger.warning(
                "Vector search failed (index may not be configured)",
                error=str(e)
            )
            return []
