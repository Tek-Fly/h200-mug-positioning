...
from src.core.analyzer import H200ImageAnalyzer, AnalysisResult, BatchResult
...
class AsyncProcessingPipeline:
        ...
        try:
            ...
            if len(job.images) == 1:
                ...
                result = await self.analyzer.analyze_batch(
                    job.images,
                    metadata_list=[job.metadata] * len(job.images) if job.metadata else None
                )
            ...
            try:
                ...
                # Scale down if low pressure
                    ...
                    for worker_id, stats in self._worker_stats.items():
                        if stats['last_job_time'] is None or \
                           time.time() - stats['last_job_time'] > 60:
    ...
    def get_stats(self) -> QueueStats:
        ...
        processing_jobs = sum(1 for j in self._active_jobs.values() if j.status == JobStatus.PROCESSING)
        completed_jobs = sum(1 for j in self._completed_jobs if j.status == JobStatus.COMPLETED)
        failed_jobs = sum(1 for j in self._completed_jobs if j.status == JobStatus.FAILED)
