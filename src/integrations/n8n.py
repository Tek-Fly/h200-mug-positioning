...
async def main():
    ...
    async with N8NClient() as client:
        ...
        print(f"Detection event sent: {detection_result}")
        ...
        print(f"Analysis event sent: {analysis_result}")
        
        # Example 3: Queue multiple events and flush
        await client.queue_event(
            N8NEventType.SYSTEM_ALERT,
            {"type": "performance", "cpu_usage": 78.5}
        )
        await client.queue_event(
            N8NEventType.MODEL_UPDATED,
            {"model": "yolo", "version": "1.2.0"}
        )
        
        # Flush queued events
        flush_result = await client.flush_events()
        print(f"Flushed events: {flush_result}")
        ...
        error_result = await client.send_error(
            ...
            context={
                "model_path": "/models/yolo.pth",
                "error_code": "MODEL_NOT_FOUND"
            },
            severity="critical"
        )
        print(f"Error event sent: {error_result}")
