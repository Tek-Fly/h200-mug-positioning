...
class TemplatedClient:
    ...
    async def _make_request(
        ...
        timeout = aiohttp.ClientTimeout(
            total=kwargs.pop("timeout", self.DEFAULT_TIMEOUT)
        )
        ...
        try:
            async with self.session.request(
                ...
                if response.status >= 400:
                    ...
                    raise TemplatedAPIError(f"API error {response.status}: {error_msg}")
    ...
    async def list_templates(
        ...
        result = await self._make_request("GET", "/templates", params=params)
    ...
    async def create_custom_template(
        ...
        result = await self._make_request("POST", "/templates", json=request_data)

        logger.info(f"Created custom template: {result.get('template_id')}")
        return result

    async def batch_render(self, renders: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        result = await self._make_request("POST", "/batch-render", json=request_data)
...
async def main():
    ...
    async with TemplatedClient() as client:
        ...
        positioning_data = {
            ...
            "confidence": 0.92,
            "bounding_box": {"x1": 150, "y1": 200, "x2": 270, "y2": 380},
            "template_params": {
        ...
        logger.info(f"Rendered design: {result}")
        ...
        logger.info(f"Batch rendered {len(batch_results)} designs")
