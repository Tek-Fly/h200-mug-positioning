...
from .templated import TemplatedAPIError, TemplatedClient
...
class TestN8NClient:
    ...
    async def test_send_event(self, mock_session):
        ...
        with patch.dict(
            "os.environ", {"N8N_WEBHOOK_URL": "https://test.n8n.com/webhook"}
        ):
            client = N8NClient()
            client.session = mock_session

            result = await client.send_event(
                N8NEventType.MUG_DETECTED, {"image_id": "test_123", "mug_count": 2}
            )
    ...
    async def test_enrich_event(self):
        ...
        with patch.dict(
            ...
            enriched = client._enrich_event(N8NEventType.MUG_DETECTED, {"test": "data"})
...
class TestIntegrationManager:
    ...
    def mock_clients(self):
        ...
        return {"templated": templated, "n8n": n8n, "notifications": notifications}
