...
class H200IntegrationExample:
            ...
            if health.get("templated", {}).get("healthy"):
                ...
                try:
                    if manager.templated_client:
                        ...
                        logger.info(f"✓ Render created: {render_result.get('render_url', 'N/A')}")
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            logger.info("=" * 30)
            logger.info(f"Notifications sent: {status['notifier']['metrics']['notifications_sent']}")
            logger.info(f"WebSocket notifications: {status['notifier']['metrics']['websocket_notifications_sent']}")
            logger.info(f"External notifications: {status['notifier']['metrics']['external_notifications_sent']}")
            logger.info(f"Failed notifications: {status['notifier']['metrics']['failed_notifications']}")
            logger.info(f"Average response time: {status['notifier']['metrics']['average_response_time_ms']:.2f}ms")
