...
class R2StorageClient:
    ...
    def __init__(
        ...
        if not self.secret_access_key:
            raise ValueError("R2 secret access key not provided or found in environment")
        ...
        try:
            async with self.get_client() as client:
                ...
                if health_status["bucket_exists"]:
                    try:
                        location = await client.get_bucket_location(Bucket=self.bucket_name)
                        health_status["bucket_location"] = location.get('LocationConstraint', 'us-east-1')
                    except:
                        pass
                ...
                health_status["stats"]["total_buckets"] = len(response.get('Buckets', []))
                
                logger.info("R2 storage health check passed", bucket_exists=health_status["bucket_exists"])
    ...
    async def create_bucket_if_not_exists(self, bucket_name: Optional[str] = None) -> bool:
        """
