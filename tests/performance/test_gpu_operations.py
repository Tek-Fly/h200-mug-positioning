...
class TestGPUPerformance(PerformanceTestBase):
    ...
    @requires_gpu
    @pytest.mark.asyncio
    async def test_yolo_inference_performance(
        self, sample_image, performance_thresholds
    ):
        ...
        for model_size in model_sizes:
            ...
            with patch("ultralytics.YOLO") as mock_yolo_class:
                ...
                assert (
                    avg_time <= expected_max_times[model_size]
    ...
    @requires_gpu
    @pytest.mark.asyncio
    async def test_clip_encoding_performance(
        self, sample_image, performance_thresholds
    ):
        ...
        for model_name in clip_models:
            ...
            with patch("clip.load") as mock_clip_load:
                ...
                model_key = model_name
                assert (
                    avg_time <= expected_max_times[model_key]
    ...
    async def test_batch_processing_performance(self, generate_test_images):
        ...
        for batch_size in batch_sizes:
            ...
            with patch("src.core.models.manager.ModelManager") as mock_manager_class:
                ...
                self.record_metric(
                    f"batch_{batch_size}_time_per_image", time_per_image_ms
                )
                ...
                if batch_size > 1:
                    ...
                    assert (
                        time_per_image_ms < individual_time_estimate * 0.8
        ...
        for batch_size in batch_sizes:
            ...
            with patch("src.core.models.manager.ModelManager") as mock_manager_class:
                ...
                mock_clip.encode_image_batch = Mock(
                    return_value=torch.randn(batch_size, 512)
                )
                ...
                # Record metrics
                self.record_metric(
                    f"memory_batch_{batch_size}_total_mb", memory_used_mb
                )
                self.record_metric(
                    f"memory_batch_{batch_size}_per_image_mb", memory_per_image_mb
                )

                # Memory usage assertions
                max_memory_per_image = 100  # MB per image
                assert (
                    memory_per_image_mb <= max_memory_per_image
                ), f"Memory usage too high: {memory_per_image_mb}MB per image"

                # Memory usage should scale reasonably with batch size
                if batch_size > 1:
                    expected_max_total = (
                        batch_size * max_memory_per_image * 1.2
                    )  # 20% overhead
                    assert (
                        memory_used_mb <= expected_max_total
    ...
    async def test_cold_start_performance(self, sample_image, performance_thresholds):
        ...
        with patch("src.core.models.manager.ModelManager") as mock_manager_class:
            ...
            assert (
                cold_start_time_ms <= performance_thresholds["cold_start_ms"]
            ), f"Cold start time {cold_start_time_ms}ms exceeds threshold {performance_thresholds['cold_start_ms']}ms"

            assert (
                warm_start_time_ms <= performance_thresholds["warm_start_ms"]
            ...
            assert (
                warm_start_time_ms <= cold_start_time_ms * 0.2
    ...
    async def test_concurrent_processing_performance(self, generate_test_images):
        ...
        assert (
            std_dev <= avg_individual_time * 0.3
...
class TestGPUResourceManagement(PerformanceTestBase):
    ...
    async def test_gpu_memory_cleanup(self, generate_test_images):
        ...
        for batch_size in [4, 8, 16, 8, 4]:  # Varying batch sizes
            ...
            # Record memory usage
            self.record_metric(
                f"gpu_memory_batch_{batch_size}_mb", memory_increase / (1024 * 1024)
            )
    ...
    async def test_scalability_limits(self):
        ...
        for num_requests in request_sizes:
            ...
            if num_requests > 1:
                ...
                assert (
                    processing_time_ms <= expected_linear_scaling * 2
