...
class AsyncBaseTest(BaseTest):
    ...
    async def wait_for_condition(self, condition_func, timeout_s: float = 5.0, 
...
class APITestBase(BaseTest):
    ...
    def assert_api_response_format(self, response_data: Dict[str, Any], 
                                  required_fields: List[str]):
        """Assert API response has required format."""
        for field in required_fields:
            assert field in response_data, f"Missing required field: {field}"
    
    def assert_error_response(self, response_data: Dict[str, Any], 
                            expected_code: str):
...
class PerformanceTestBase(AsyncBaseTest):
    ...
    def assert_performance_targets(self, targets: Dict[str, float]):
        ...
        for metric_name, target_value in targets.items():
            ...
            assert metric is not None, f"Performance metric '{metric_name}' not recorded"
