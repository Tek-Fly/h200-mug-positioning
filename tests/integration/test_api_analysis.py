...
class TestAnalysisAPI(APITestBase, IntegrationTestBase):
    ...
    def test_analyze_image_success(
        self, test_client, auth_headers, sample_analysis_result
    ):
    ...
    def test_analyze_image_no_file(self, test_client, auth_headers):
        ...
        response = test_client.post(
            f"{self.base_url}/analyze/with-feedback", headers=auth_headers
        )
    ...
    def test_analyze_batch_success(
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            mock_batch_result.results = [
                AsyncMock(to_dict=lambda: sample_analysis_result)
            ] * 2
            ...
            response = test_client.post(
                f"{self.base_url}/analyze/batch", files=files, headers=auth_headers
            )
    ...
    def test_get_analysis_result_success(
        ...
        with patch("src.database.mongodb.get_mongodb_client") as mock_get_db:
            ...
            response = test_client.get(
                f"{self.base_url}/analyze/results/{analysis_id}", headers=auth_headers
            )
    ...
    def test_analyze_image_with_options(
        self, test_client, auth_headers, sample_analysis_result
    ):
    ...
    def test_reanalyze_existing_image(
        self, test_client, auth_headers, sample_analysis_result
    ):
...
class TestAnalysisAPIPerformance(APITestBase):
    ...
    def test_analyze_image_performance_threshold(
        self, test_client, auth_headers, performance_thresholds
    ):
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            assert (
                ...
                <= performance_thresholds["image_processing_ms"]
            )
    ...
    def test_batch_analysis_scaling(self, test_client, auth_headers):
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            def mock_batch_analysis(images):
                ...
                mock_batch_result.results = [
                    AsyncMock(to_dict=lambda: {}) for _ in images
                ]
