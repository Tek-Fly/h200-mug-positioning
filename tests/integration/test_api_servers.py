...
class TestServersAPI(APITestBase, IntegrationTestBase):
    ...
    def test_get_server_metrics(self, test_client, auth_headers):
        ...
        with patch('src.control.manager.metrics.get_server_metrics') as mock_get_metrics:
    ...
    def test_get_server_health(self, test_client, auth_headers):
        ...
        with patch('src.control.manager.orchestrator.get_orchestrator') as mock_get_orch:
            ...
            response = test_client.get(
                f"{self.base_url}/servers/{server_type}/health",
                headers=auth_headers
            )
    ...
    def test_server_not_found(self, test_client, auth_headers):
        ...
        response = test_client.get(
            f"{self.base_url}/servers/{invalid_server_type}",
            headers=auth_headers
        )
...
class TestServersWebSocket(IntegrationTestBase):
    ...
    def test_server_logs_websocket(self, test_client):
        ...
        with patch('src.control.api.routers.websocket.LogStreamManager') as mock_manager_class:
    ...
    def test_server_metrics_websocket(self, test_client):
        ...
        with patch('src.control.api.routers.websocket.MetricsStreamManager') as mock_manager_class:
            mock_manager = AsyncMock()
            mock_manager_class.return_value = mock_manager
