...
class TestCompleteImageAnalysisWorkflow(E2ETestBase):
        ...
        workflow_steps = [
            {
                "type": "upload_image",
                "data": {
                    "image": self.test_images["simple_mug"],
                    "name": "simple_mug.png",
                },
                "delay": 0.1,
            },
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            with patch("src.database.mongodb.get_mongodb_client") as mock_get_db:
                ...
                response = test_client.get(
                    f"/api/v1/analyze/results/{analysis_id}", headers=auth_headers
                )
    ...
    async def test_batch_analysis_workflow(self, test_client, auth_headers):
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            for i, (name, image) in enumerate(self.test_images.items()):
                ...
                mock_result.to_dict.return_value = {
                    ...
                    "mug_positions": (
                        []
                        if name == "no_mug"
                        else [{"x": 200, "y": 250, "confidence": 0.8}]
                    ),
            ...
            response = test_client.post(
                "/api/v1/analyze/batch", files=files, headers=auth_headers
            )
            ...
            results_by_index = {
                i: result for i, result in enumerate(batch_data["results"])
            }
    ...
    async def test_rules_integration_workflow(self, test_client, auth_headers):
        ...
        with patch("src.core.rules.engine.RulesEngine") as mock_engine_class:
            ...
            # Create rule
            response = test_client.post(
                "/api/v1/rules", json=rule_data, headers=auth_headers
            )
            ...
            with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
                ...
                mock_result.to_dict.return_value = {
                    ...
                    "mug_positions": [{"x": 250, "y": 275, "confidence": 0.92}],
                ...
                response = test_client.post(
                    "/api/v1/rules/evaluate", json=analysis_data, headers=auth_headers
                )
    ...
    async def test_performance_monitoring_workflow(self, test_client, auth_headers):
        ...
        with patch("src.core.analyzer.H200ImageAnalyzer") as mock_analyzer_class:
            ...
            assert all(
                latency <= 500 for latency in api_latencies
            ), "API latencies too high"

            # Processing times should improve with warm-up
            assert (
                processing_times[-1] <= processing_times[0]
            ), "Processing time should improve"

            # Cached requests should be faster
            cached_times = [
                p["processing_time_ms"] for p in performance_data if p["cached"]
            ]
            uncached_times = [
                p["processing_time_ms"] for p in performance_data if not p["cached"]
            ]
...
class TestServerLifecycleWorkflow(E2ETestBase):
    ...
    async def test_server_start_stop_workflow(self, test_client, auth_headers):
        ...
        with patch(
            ...
            response = test_client.get(
                f"/api/v1/servers/{server_type}", headers=auth_headers
            )
...
class TestExternalServicesWorkflow(E2ETestBase):
    ...
    async def test_notification_integration_workflow(
        self, test_client, auth_headers, mock_webhook_server
    ):
        ...
        with patch(
            "src.integrations.notifications.NotificationManager"
        ) as mock_notif_class:
