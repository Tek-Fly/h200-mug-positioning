...
class MockR2StorageClient:
    ...
    async def get_object(self, bucket, key):
        ...
        body_stream = io.BytesIO(obj["content"] if isinstance(obj["content"], bytes) else obj["content"].encode())
    ...
    async def list_objects_v2(self, bucket, prefix=None, max_keys=1000):
        ...
        self._record_api_call("list_objects_v2", bucket=bucket, prefix=prefix, max_keys=max_keys)
    ...
    async def start(self):
        ...
        async def webhook_handler(request):
            ...
            try:
                ...
                return web.json_response({"status": "received", "id": len(self.received_webhooks)})
...
class ExternalServiceManager:
    ...
    async def simulate_deployment_workflow(self):
        ...
        await self.r2_storage.put_object("config-bucket", f"{pod_id}/config.json", config_data)
        
        # Send notification
        await self.notifications.send_webhook_notification(
            "http://localhost:8080/webhook",
            {"event": "pod_created", "pod_id": pod_id}
        )
