...
from src.core.analyzer import H200ImageAnalyzer, AnalysisResult, BatchAnalysisResult
...
class TestH200ImageAnalyzer(ModelTestBase):
    ...
    async def test_analyze_image_success(self, sample_mug_image):
        ...
        self.mock_model_manager.get_model.return_value.encode_image.return_value = mock_embedding
...
class TestAnalyzerPerformance(ModelTestBase):
    ...
    @pytest.mark.asyncio
    async def test_analysis_performance_threshold(self, sample_mug_image, performance_thresholds):
        """Test analysis meets performance thresholds."""
        analyzer = H200ImageAnalyzer()
        analyzer._initialized = True
        
        # Mock fast responses
        with patch.object(analyzer, '_run_yolo_inference', return_value=[]), \
             patch.object(analyzer, '_run_clip_inference', return_value=np.random.randn(512)), \
             patch.object(analyzer.positioning_engine, 'calculate_positions', return_value=[]):
