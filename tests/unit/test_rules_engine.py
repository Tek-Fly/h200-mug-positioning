...
from src.core.rules.models import Action, Condition, Rule, RuleType
...
class TestRulesEngine(AsyncBaseTest):
    ...
    async def test_create_rule_from_text_success(self):
        ...
        mock_rule = Rule(
            ...
            conditions=[
                Condition(field="mug_confidence", operator="greater_than", value=0.8)
            ],
            actions=[Action(type="highlight_position", parameters={"style": "bold"})],
    ...
    async def test_evaluate_rules_success(self):
        ...
        analysis_data = {"mug_confidence": 0.9, "position_x": 150, "position_y": 200}
...
class TestRuleExecutor(AsyncBaseTest):
    ...
    async def test_evaluate_rules_success(self):
        ...
        analysis_data = {"confidence": 0.9, "position_x": 150, "mug_detected": True}
    ...
    async def test_evaluate_single_rule_true_condition(self):
        ...
        with patch.object(
            self.executor, "_execute_actions", return_value=["highlight"]
        ) as mock_execute:
    ...
    def test_evaluate_condition_greater_than(self):
        ...
        assert (
            self.executor._evaluate_condition(condition, {"confidence": 0.7}) is False
        )
    ...
    def test_evaluate_condition_less_than(self):
        ...
        assert (
            self.executor._evaluate_condition(condition, {"position_x": 150}) is False
        )
    ...
    def test_evaluate_condition_equals(self):
        ...
        assert (
            self.executor._evaluate_condition(condition, {"status": "detected"}) is True
        )
        assert (
            self.executor._evaluate_condition(condition, {"status": "not_detected"})
            is False
        )
    ...
    def test_evaluate_condition_contains(self):
        ...
        assert (
            self.executor._evaluate_condition(condition, {"tags": ["mug", "kitchen"]})
            is True
        )
        assert (
            self.executor._evaluate_condition(condition, {"tags": ["cup", "table"]})
            is False
        )
    ...
    def test_evaluate_condition_missing_field(self):
        ...
        assert (
            self.executor._evaluate_condition(condition, {"other_field": "value"})
            is False
        )
    ...
    async def test_execute_actions(self):
        ...
        with (
            patch.object(self.executor, "_execute_highlight_action") as mock_highlight,
...
class TestRuleModels:
    ...
    def test_create_rule(self):
        ...
        rule = Rule(
            ...
            actions=[Action(type="highlight", parameters={"color": "red"})],
            created_at=datetime.now(),
            active=True,
        )
